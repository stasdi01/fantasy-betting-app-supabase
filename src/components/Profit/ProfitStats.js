...
const ProfitStats = ({ tickets }) => {
  // Izračunaj statistike koristeći SAČUVANI status
  const calculateStats = () => {
    ...
    tickets.forEach((ticket) => {
      ...
      const status = ticket.status || "pending"; // Koristi sačuvani status
    ...
    const roi =
      totalStaked > 0 ? ((netProfit / totalStaked) * 100).toFixed(2) : 0;
    const winRate =
